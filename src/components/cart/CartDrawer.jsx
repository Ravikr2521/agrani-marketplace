...
export default function CartDrawer({ open, onOpenChange }) {
  ...
  useEffect(() => {
    if (step === 2 && open) loadAddresses();
  }, [step, open, AgraniToken]);
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      localStorage.setItem("farmers_marketplace_buyer_phone", verifiedMobile);
                      ...
                      {addressesLoading ? (
                        ...
                        <>
                          ...
                          >
                            <SelectTrigger className="h-11 w-full rounded-xl bg-white">
                              <SelectValue
                                placeholder={t("Choose a saved address")}
                              />
                            </SelectTrigger>
                              ...
                              {addresses.map((address) => (
                                <SelectItem key={address.id} value={address.id}>
                                  <span className="font-medium">
                                    {t(
                                      String(address.address_type || "Address"),
                                    )}
                                  </span>{" "}
                        ...
                        <div className="rounded-xl border border-dashed border-emerald-200 bg-emerald-50/50 px-4 py-5 text-center">
                          ...
                          <Button
                            type="button"
                            size="sm"
                            className="mt-3 h-8 rounded-lg border border-gray-300/70 bg-white px-4 text-xs text-green-800 shadow-none hover:bg-white! hover:shadow-sm"
                            onClick={() => setAddressDialogOpen(true)}
                          >
